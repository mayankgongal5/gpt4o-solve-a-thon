...
class _SearchStocksPageState extends State<SearchStocksPage> {
  ...
  DateTime? _endDate;
  List<FlSpot> _chartData = [];
  final String _apiKey = 'TU9PEVPG88SA73IH'; // Replace with your Alpha Vantage API key
  final List<Map<String, String>> _trendingStocks = [
    {'symbol': 'ZOMATO', 'name': 'Zomato'},
    {'symbol': 'ZERODHA', 'name': 'Zerodha'},
    ...
    {'symbol': 'VODAFONE', 'name': 'Vodafone Idea'},
  ];
  ...
  Future<void> _searchStocks() async {
    ...
    final String url =
        'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=$symbol.BSE&apikey=$_apiKey'; // Change to Alpha Vantage API
    ...
    if (response.statusCode == 200) {
      ...
      if (data.containsKey('Time Series (Daily)')) {
        ...
        timeSeries.forEach((date, values) {
          if (DateTime.parse(date).isAfter(_startDate!) && DateTime.parse(date).isBefore(_endDate!)) {
            final double closePrice = double.parse(values['4. close']);
            chartData.add(FlSpot(index.toDouble(), closePrice));
            index++;
          }
        });
